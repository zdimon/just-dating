...
class Room(models.Model):
    '''
        Викторины, которые активны в настоящее время. Используются для переодического опроса и перехода на новый вопрос, если вышло время ответа. Так же для завершения викторины по времени или по окончанию очереди вопросов.
    '''
   


    type = models.CharField(verbose_name=_(u'Quiz type'), max_length=10, choices=ROOM_TYPES, default='questionend')
    ...
    questions_json = JSONField(verbose_name=_(u"List of questions indexes"), default={})
    winner = models.ForeignKey(User, verbose_name=_(u'Winner'), null=True, blank=True)
